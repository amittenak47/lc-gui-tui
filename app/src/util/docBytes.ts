...
/**
 * What a range of bytes looks like to a PDF reader, decided here rather than
 * by pdf.js.
 *
 * pdf.js answers two very different situations with one sentence. `XRef.parse`
 * swallows *every* exception thrown while it reads the cross-reference table —
 * it reports it through `info()`, which is silent at the default verbosity —
 * then retries in recovery mode, and when the recovery scan also comes up
 * without a trailer it throws `Invalid PDF structure.` So a file that arrived
 * half-downloaded and a runtime fault inside the worker (a WebView missing an
 * API the bundle assumed it had) surface as the same string, and the advice
 * those two need is opposite: pick the file again, versus the file is fine and
 * the app is not.
 *
 * This settles the half that can be settled locally. A PDF opens with `%PDF-`
 * and closes with `%%EOF`; a copy that has both is not truncated, whatever
 * pdf.js says about it next.
 */
export interface PdfByteReport {
  /** True when the bytes are shaped like a whole PDF. */
  whole: boolean;
  /** Readable summary — size, header, trailer — for an error the reader sees. */
  detail: string;
}

/** How far back from the end `%%EOF` is looked for. */
...
function readableSize(n: number): string {
...
/**
 * Say what a run of bytes is, when it is not the PDF it was filed as.
 *
 * "Not a PDF" is where the last round of this stalled: the message named what
 * the bytes were not, and the only lead left was to guess. A JSON envelope, an
 * HTML error page and a truncated download all need different answers, and all
 * three announce themselves in their first few characters — so print them.
 */
function describeNonPdf(view: Uint8Array): string {
  const head = ascii(view.subarray(0, Math.min(view.length, 200)));
  const trimmed = head.trimStart();
  const preview = JSON.stringify(
    trimmed.slice(0, 80).replace(/\s+/g, " ").trim(),
  );
  if (/^<!doctype html|^<html/i.test(trimmed)) {
    return `it is an HTML page, which usually means a server answered instead of sending the file: ${preview}`;
  }
  if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
    return `it is JSON, not a document — something answered a request for the file with a message: ${preview}`;
  }
  if (view[0] === 0x50 && view[1] === 0x4b) {
    return "it is a ZIP — an EPUB or an archive that was opened as a PDF";
  }
  if (view[0] === 0x1f && view[1] === 0x8b) {
    return "it is gzip-compressed and was never decompressed";
  }
  return `it begins: ${preview}`;
}

export function pdfByteReport(bytes: ArrayBuffer): PdfByteReport {
  const view = new Uint8Array(bytes);
  const size = readableSize(view.length);
  if (view.length === 0) return { whole: false, detail: "the copy is empty" };
  const head = ascii(view.subarray(0, Math.min(view.length, 9)));
  if (!head.startsWith("%PDF-")) {
    return {
      whole: false,
      detail: `${size}, and it does not begin with %PDF — ${describeNonPdf(view)}`,
    };
  }
  const version = head.slice(0, 8).trim();
  const tailFrom = Math.max(0, view.length - PDF_TRAILER_WINDOW);
  const hasEof = ascii(view.subarray(tailFrom)).includes("%%EOF");
  if (!hasEof) {
    return {
      whole: false,
      detail: `${size}, ${version} header, but no %%EOF at the end — the copy is truncated`,
    };
  }
  return { whole: true, detail: `${size}, ${version} header, %%EOF present` };
}
