...
export function DocumentExportDialog({name,docType,onExport,onClose}:Props) {
  const [options,setOptions]=useState<DocumentExportOptions>({ink:true,footnotes:true,threads:false});
  ...
  const format=docType==="pdf" ? "PDF":docType==="epub" ? "EPUB":"Markdown + images (ZIP)";
  return <div className="lc-settings-backdrop">
    <div className="lc-settings-modal lc-attempt-modal lc-document-export" role="dialog" aria-modal="true" aria-labelledby="document-export-title">
      ...
      <div className="lc-settings-body">
        ...
        <p className="lc-muted">{docType==="pdf" ? "Original pages with ink and footnote markers. Full footnotes and sketches follow in an appendix.":"Readable text with handwriting images near the relevant passages and full footnotes at the end."}</p>
