...
export function LibrarySearch({value,onChange,label,disabled=false,kinds=[],filter="all",onFilterChange,showTrash=false,onTrashChange}:{
  value:string;onChange:(value:string)=>void;label:string;disabled?:boolean;
  kinds?:string[];filter?:string;onFilterChange?:(value:string)=>void;
  ...
  return <div className="lc-library-search">
    ...
    <div className="lc-library-search-filters" role="group" aria-label="Filter saved items">
      {kinds.map(kind=><button key={kind} type="button" disabled={disabled} title={kind === "markdown" ? "Markdown" : kind.toUpperCase()} aria-label={`Filter ${kind}`} aria-pressed={filter===kind} onClick={()=>onFilterChange?.(filter===kind ? "all" : kind)}>{kind === "markdown" ? "MD" : kind.toUpperCase()}</button>)}
