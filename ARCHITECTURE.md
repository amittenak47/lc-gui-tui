...
Internals. If you just want to install and use the app, the
...
| CLI binary | `lc` — `cargo install --path .` | [`src/`](src/) |
| GUI crate | `whiteboard-gui` | [`app/src-tauri/Cargo.toml`](app/src-tauri/Cargo.toml) |
| GUI native lib | `whiteboard_lib` — what Android links as `libwhiteboard_lib.so` | same |
...
The desktop window **is** the router. Tauri holds axum in-process — no TCP
bind, no `127.0.0.1:7878`, no separate `lc serve` step — and runs tests with
...
| **Harness** | Corpus, `solution.py`, RustPython tests, document index | Inside the GUI process | Workspaces + `problems.db` on this machine |

Same React client on desktop, Android, and in Vite. Vite-in-a-browser
(`npm run dev`) has no Tauri behind it and is not a supported path. To drive
the *desktop* window from a tablet, use spacedesk — that is pixels only, not a
second instance.
...
| --- | --- | --- |
| **Whiteboard / Annotate** | Full. The working copy is IndexedDB on the device. | Dual-writes to `pads.db` + `pad-blobs/`. A tombstone hides a notebook on every device sharing that store; snapshots and PDF bytes stay with it. The sidecar `.lc-ink.json` is a backup, not the sync path. |
| **Problems** | Empty until Settings → Datasets → Install. Tests run in-process. Pass/fail lives in `session.json` and survives Remove + reinstall. | Anything sharing the workspace dir shares the state. On reconnect, Personalise `offlineMerge` (ask / prefer-local / prefer-server) decides which board wins. |
...
is hold-to-confirm and only tombstones the live list — restore from the archive
or from the 2h / 24h / 7d snapshots.

Personalise (handedness, theme, capture folder, …) is a per-device blob.
...
- **The notebook library** — `pads.db`, `pad-blobs/`, `/pads/…`,
  [`src/pads.rs`](src/pads.rs), `PadKind`. **Not renamed.** These name data on
  disk in every existing install; changing them is a runtime migration, not a
...
The reference solution shipped with a corpus is never loaded into a prompt.
Review runs perceive → claim → verdict inside the process, and answers over
Tauri events (`lc-coach-frame`) so each stage appears as it happens.

HTTP routes stay `/coach/*` and config keys stay `coach.*` and
...
The Practice APK is aarch64-only: rustpython 0.5 does not compile for 32-bit
Android. Whiteboard-only has no such limit and builds universal.
...
One JSON object per problem whose tests RustPython cannot execute. Flushes
every 25 rows; `--resume` continues after a crash.

Dataset adapters: [`src/datasets/`](src/datasets/). After changing one:
`whiteboard index --dataset <slug> --rebuild`.
...
is a **different product**, not a branch waiting to merge. No corpus, no
RustPython runner, no problem browser. Tauri depends on the crate with
`default-features = false`: agent in the APK, no axum. Ask talks straight to
`llm.local.base_url`.

Staged Review does not exist there — Ask is one model call. Draw/Viz still has
a tool loop for diagrams, which is not perceive → claim → verdict.

This tree gates Practice behind `VITE_FEATURE_LEETCODE` + the Cargo `leetcode`
...
- **`lc sync` hub** — notebooks across devices.
